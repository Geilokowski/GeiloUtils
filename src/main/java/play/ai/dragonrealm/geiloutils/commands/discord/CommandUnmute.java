...
public class CommandUnmute extends CommandBase {
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        if(args.length < 1){
            throw new WrongUsageException("/discordunmute <Server_bot_name>", new Object[0]);
        }
        if(sender instanceof EntityPlayer) {
            ...
            if (DiscordBotMain.getInstance().getBotsInGuild(false).contains(botName)) {
                if(GeiloUtils.getManager().getConfig(PlayerStatsConfig.class).removeFromMuteList(player.getCachedUniqueIdString(), botName)) {
                    sender.sendMessage(new TextComponentString("[GeiloBot] Unmuted " + botName + " chat."));
                } else {
                    sender.sendMessage(new TextComponentString("[GeiloBot] Channel was not muted!"));
                }
